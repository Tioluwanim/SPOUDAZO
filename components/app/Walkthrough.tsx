...
function cardStyle(rect: DOMRect | null, placement: WalkthroughStep["placement"]): CSSProperties {
  ...
  const clampLeft = Math.max(GAP, Math.min(rect.left, vw - CARD_WIDTH - GAP));
  const clampTop = Math.max(GAP, Math.min(rect.top, vh - 220));

  switch (placement) {
    case "top":
      return { left: clampLeft, bottom: vh - rect.top + GAP };
    case "left":
      return { right: vw - rect.left + GAP, top: clampTop };
    case "right":
      return { left: rect.right + GAP, top: clampTop };
    case "bottom":
    default:
    ...
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="absolute rounded-2xl border border-ink-border bg-ink-soft/90 p-5 shadow-2xl backdrop-blur-xl"
          style={{ width: CARD_WIDTH, ...cardStyle(rect, step.placement) }}
        >
          <button
            onClick={finish}
